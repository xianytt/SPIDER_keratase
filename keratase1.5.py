...
class Spider_kerastase(object):
    ...
    def insert_brand(self, brand_name, brand_logo):
        ...
        with open('F:\python_project\kerastase\logo_{}.png'.format(brand_name), 'wb') as f:
            f.write(r.content)
        f = open('F:\python_project\kerastase\logo_{}.png'.format(brand_name), 'rb')
        brand_logo = f.read()
        f.close()
    ...
    def save_data(self, data_dict):
        ...
        if data_dict['good_type'] == []:
            data_dict['good_type'] = "NULL"
        #汇率处理
        if data_dict['price']:
